...
| `POST_CHECK_DELAY_SEC` | Отложенная финальная перепроверка уже прошедших ключей (сек.; `0` = выкл, `300` = перепроверка через ~5 минут) |
| `POST_CHECK_WORKERS` | Потоки для финальной перепроверки |
